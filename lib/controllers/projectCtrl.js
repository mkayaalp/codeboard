...
var db = require('../models'),
  ...
  ErrorWithStatus = require('../error.js').ErrorWithStatus,
...
var getAllProjects = function (req, res) {
  ...
  for (var i = 0; i < searchTerms.length; i++) {
    ...
    var w = {[Op.or]: [
      {projectname: {[Op.substring]: searchTerms[i]}},
      {description: {[Op.substring]: searchTerms[i]}},
      {language: {[Op.substring]: searchTerms[i]}}
    ]};
...
var putProjectSettings = function (req, res) {
  ...
  if (valErrors) {
    res.status(400).send('There have been validation errors: ' + util.inspect(valErrors));
    return;
  }
...
var putProject = function (req, res) {
  ...
  db.Project
    ...
    .then(function () {
      ...
      return project.update({lastUId: req.body.project.lastUId});
    })
    .then(function () {
    ...
    db.User.findAll(
      ...
      .then(function (usr) {
        ...
        if(usr.length > 0) {
          ...
          res.status(404).json({msg: 'The requested user does not exist'});
        }
      })
      .catch(function (err) {
...
var _runActionOnProject = function (req, res) {
  ...
  if (action === 'compile') {
    mantraSrv.compile(data)
      ...
      .catch(ErrorWithStatus, function(err) {
        logSrv.addActionLog(req, start, data, " error compiling " + JSON.stringify(err));
        res.status(err.statusCode).json({output: err.message});
      })
      .catch(function (err) {
  ...
  db
    ...
    .then(function (prj) {
      ...
      if (!prj.isLtiAllowed || !isRequestAuthorized(req, prj.ltiSecret)) {
        ...
        throw new ErrorWithStatus(401, 'The project does not allow access via LTI or the provided key/secret combination is wrong.');
      }
    ...
    .then(function (ltiSession) {
      ...
      if(ltiSession == null) {
        throw new ErrorWithStatus(500, 'Error while creating an LTI session in the database.');
      }
  ...
  projectSrv
    ...
    .then(function (compilationResult) {
      ...
      if(compilationResult.compilationError) {
        throw new Error('Submission failed. Your program does not compile.\nFix all compilation errors and submit again.');
      }
...
var getAllUserProjectsForProject = function(req, res) {
  ...
  db.UserProject
    .findAll({
      ...
      order: [[{mode: db.User, as: 'user'}, 'username', 'ASC']],
      include: [
        {
          model: db.User,
          as: 'user',
  ...
  db.UserProject
    ...
    .then(function(aFullUserProject){
      if(!(aFullUserProject)) {
        ...
        throw new ErrorWithStatus(500, 'Error retrieving userProject with id ' + _userprojectId + ' for project with id ' + _projectId);
      }
