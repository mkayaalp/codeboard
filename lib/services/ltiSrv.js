...
var db = require('../models'),
  ...
  request = Promise.promisifyAll(require('request'), {multiArgs: true}); // use bluebird promises for requests
  ...
  return db.Project
    ...
    .then(function(prj) {
      ...
      if(prj !== null && !prj.isLtiAllowed) {
        throw new Error('Invalid project or project has no LTI support enabled.');
      }
    ...
    .then(function(ltiSession) {
      ...
      if(ltiSession === null) {
        throw new Error('No valid LTI session for {ltiSession: ' + ltiSessionId +', ltiUser: ' + ltiUser + ', ltiNonce: ' + ltiNonce + '}.\n' +
    ...
    .then(function (res) {
      ...
      if(res[0].statusCode !== 200) {
       throw new Error('Error while submitting grade to LTI TC platform. Try to submit again or report the problem to info@codeboard.io: \n' + res[1]);
      }
