...
app.post('/compile', function(req, res) {
  ...
  if (data.clean==='true') {
    res.status(200).send('Mocking CLEAN Compilation succeeded of project: '+data.id+' path '+data.path+' target '+data.target);
  }
  else {
    res.status(200).send('Mocking  Compilation succeeded of project: '+data.id+' path '+data.path+' target '+data.target);
  }
...
app.get('/run', function(req, res) {
  var id = req.query.id;
  res.status(200).send('Mocking Running project: '+id);
...
app.get('/interfaceView', function(req, res) {
  ...
  res.status(200).send('Mocking interface view project: '+id+" class: "+className);
...
app.get('/flatView', function(req, res) {
  var id = req.query.id;
  var className = req.query.class;
  res.status(200).send('Mocking Flat view project: '+id+" class: "+className);
...
app.get('/contractView', function(req, res) {
  ...
  var className = req.query.class;
  res.status(200).send('Mocking Contract view project: '+id+" class: "+className);
...
app.get('/classDescendants', function(req, res) {
  var id = req.query.id; // ID in Mantra
  var className = req.query.class;
  res.status(200).send('Mocking Class Descendants in project: '+id+" class: "+className);
...
app.get('/classAncestors', function(req, res) {
  var id = req.query.id; // ID in Mantra
  var className = req.query.class;
  res.status(200).send('Mocking Class Ancestors in project: '+id+" class: "+className);
...
app.get('/classClients', function(req, res) {
  var id = req.query.id; // ID in Mantra
  var className = req.query.class;
  res.status(200).send('Mocking Class Clients in project: '+id+" class: "+className);
...
app.get('/classSuppliers', function(req, res) {
  var id = req.query.id; // ID in Mantra
  var className = req.query.class;
  res.status(200).send('Mocking Class Suppliers in project: '+id+" class: "+className);
...
app.get('/featureCallers', function(req, res) {
  ...
  res.status(200).send('Mocking Class feature callers in project: '+id+" class: "+className+" feature "+featureName);
...
app.get('/commandLine', function(req, res) {
  ...
  var featureName = req.query.feature;
  res.status(200).send('Mocking Command line compiler, ID: '+id+" arguments: "+commandLine);
