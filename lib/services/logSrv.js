...
var compilerModel = mongoose.model('compilerlog', compilerSchema);
...
var showCompilationPerDayLogs = function (startDate, endDate, projectId, res) {
  ...
  var result = {};
  var filter = [{endTime: {$gte:inputStartDate, $lte:inputEndDate}}];

  if (projectId != undefined)
    filter.push({projectId: parseInt(projectId)});

  var filterCompiler = {$and: filter.concat({action: "compile"})};
  var filterRun = {$and: filter.concat({action: "run"})};
  var filterTest = {$and: filter.concat({action: "test"})};
  var filterTool = {$and: filter.concat({action: "tool"})};

  compilerModel.aggregate()
    .match(filterCompiler)
    .group({
      _id: {
        year: {$year: "$startTime"},
    ...
    .then(function(compilerLogs) {
      ...
      return compilerModel.aggregate()
        .match(filterRun)
        .group({
          _id: {
            year: {$year: "$startTime"},
...
var showProjectAccessPerDayLogs = function (startDate, endDate, projectId, res) {
  ...
  if (projectId != undefined)
    filter.push({projectId: projectId});

  var filterAnd = {$and: filter};
  var result = {};
  pageViewModel.aggregate()
    .match(filterAnd)
    .group({
