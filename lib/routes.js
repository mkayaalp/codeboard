...
module.exports = function(app) {
  ...
  app.put('/api/passwordReset', validator.checkEmail, authCtrl.resetPassword);


  /** Routes related to users **/

  app.get('/api/users', userCtrl.getUsers); // get profile of a user, details vary based ownership
  app.post('/api/users', validator.checkUser, userCtrl.createUser); // create a new user
  ...
  app.put('/api/users/:username/settings/password', mw.isAuth, mw.isSelf, validator.checkPassword, userCtrl.putPassword); // update the user's password
  ...
  app.put('/api/projects/:projectId/settings', mw.isValidProjectId, mw.isAuth, mw.isOwner, validator.checkProject, projectCtrl.putProjectSettings);
  ...
  app.get('/api/log/summaryDay', mw.isSuper, logCtrl.getCompilationPerDayLogs);
  app.get('/api/log/summaryHour', mw.isSuper, logCtrl.getCompilationPerHourLogs);
  app.get('/api/log/user/summaryProjectAccess', mw.isSuper, logCtrl.getProjectAccessPerUserLogs);
  app.get('/api/log/user/summaryCompiler', mw.isSuper, logCtrl.getCompilerActivityPerUserLogs);
  app.get('/api/log/user/summarySubmitAccess', mw.isSuper, logCtrl.getSubmitPerUserLogs);
  app.get('/api/logLimit/:limit', mw.isSuper, logCtrl.getLogs);
