    ...
    ltiSrv.isLtiAllowedForProject(projectId)
      .then(function(isAllowed) {
        ...
        if(isAllowed) {
          ...
          throw new Error('Access rejected because project ' + projectId + ' does not allow LTI access.');
        }
      })
      .then(function(isValidLtiUserSession) {
        ...
        if(isValidLtiUserSession) {
          ...
          throw new Error("Access rejected because user's supplied LTI session data is invalid.");
        }
      })
      .catch(function(err) {
        console.log(err);
